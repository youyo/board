...
| ステータス | M17 完了（**Phase G 1 件目・documentID discovery helper 確立**。`findAnyDocumentID(t, client, docType)` を `e2e_helpers_test.go` に追加。Unit テスト（httptest ベース: estimate found + delivery found + unknown docType doc）Green。実 API smoke test（estimate 1 docType）PASS: projectID=95944469 documentID=105287235 発見。実消費 **2 req**（`ListProjectsPage` 1 + `GetProjectWithGroupRaw` 1）、0.69 秒。helper シグネチャ: `(t *testing.T, client *boardapi.Client, docType string) (projectID, documentID int)`。probe struct 方式で全 5 docType を統一処理。`ListProjectsPage` で全ページ走査を回避（修正前 `ListProjectsRaw+WithPerPage` は ListAll で全 2405 件走査＝257 秒、本修正で解決）。） ※履歴は下記に保持 |
