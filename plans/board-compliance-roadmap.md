...
| 作成日 | 2026-04-20 |
| 最終更新 | 2026-04-20 19:10 |
| ステータス | M12 完了（**Phase E 1 件目、Get > List 情報量差モデル新発見、`Memo` 逆方向 8 件連続で BOARD API 全般仕様最終確定**。List FAIL（299 items, unmapped **15**）/ Get FAIL（**200 成功 = Phase D/E コア業務系 Get 4 件連続 200**、unmapped **29** = List 15 + Get 限定 14）/ Search FAIL（299 items, unmapped 15, name filter 無視 **7 件連続**）。Unit 5/5 Green。`ClientEntity` は 6 フィールド中 **2 つ（Code/Memo）が逆方向不整合**、**既存 Entity の根本不足が M12 で最大規模（271cba3 の 3 倍規模）**に到達。**ネスト構造は発現せず** → M11 確定「ネストは client の子リソース特有」法則に沿う（clients 自身はフラット）。Get は List より 14 フィールド多い情報リッチ応答（**新 2 段階モデル**）。実消費 4 req（見積 5 以下、pin-point accuracy）） ※履歴は下記に保持 |
| ステータス履歴 | M11 完了（**Phase D 完走、3 件連続 Get 200 確定**。List FAIL（22 items, unmapped **4** = `cost / description / invoice_date / payment_date`）/ Get FAIL（**200 成功 = Phase D コア業務系 Get 提供が 3 件連続確定**、unmapped 4 + 既存 4 フィールド逆方向不整合）/ Search FAIL（22 items, unmapped 4, `ProjectID=0` 非付与で全件返却）。Unit 5/5 Green。`ProjectCostEntity` は 8 フィールド中 **半分（4 つ）が逆方向不整合**（`Name/CostType/Amount/Memo` が実 API に不在）。**ネスト構造は発現せず** → ネストパターンは "client の子" 特有と確定。**概念モデルが根本ズレ**: Entity は「労務費/資材費の集計」想定、実 API は「仕訳的 expense entry（`description`+`cost`+`invoice_date`+`payment_date`）」。Memo 逆方向パターン **7 件連続**で全般仕様確定。実消費 4 req（pin-point accuracy）） |
...
## Current Focus
- **マイルストーン**: M13 projects 厳格突合 + GetWithGroup 全 response_group（Phase E 2 件目、複雑度最高、~15 req）
- **直近の完了**: M12 実 API E2E 実行で **Phase E 1 件目 = clients 厳格突合**: List FAIL（299 items, unmapped **15**）/ Get FAIL（**200 成功** = Phase D/E コア業務系 Get **4 件連続 200 確定**、unmapped **29** = List 15 + Get 限定 **14 フィールド**）/ Search FAIL（299 items, unmapped 15, `name` filter 無視 **7 件連続**）。実 API キー構成（トップレベル）: List/Search は **20 キー**（`id, name, address1/2, company_number, fax, invoice_system_*(4), name_disp, payment_term_id/name, pref, tel, title, zip, updated_at, created_at`）、Get は **34 キー**（List 20 + Get 限定 14 = `accounting_code, archive_flg, bank_charge_to_client_flg, basic_agreement_flg, cc, company_bank_id, company_bank_name, custom_no, document_send_type/type_name, nda_flg, note, tags, to`）。**ネスト構造は発現せず** → M11 確定「ネストは client の子リソース特有」法則に沿う。**M12 新発見**: BOARD API clients は **Get > List 情報量差** を持つ 2 段階モデル（M13 `GetWithGroup` の `response_group` パラメータ方式とは別の、自動拡張型）。既存 `ClientEntity` 6 フィールド中 **2 つ（Code/Memo）が逆方向不整合**: `Code` は `custom_no`/`accounting_code` で代替、`Memo` は `note` で代替（**`Memo` 逆方向 8 件連続で BOARD API 全般仕様として最終確定**、M03/M04/M06/M08.Name/M09/M10/M11/M12）。271cba3 UserEntity 修正の **3 倍規模**の Entity 全面改訂がフォローアップに発生。Unit 5/5 Green。403/429/リソース全体 403 は発生せず。実消費 **4 req**（見積 5 req、pin-point accuracy、上限 10 以内）。**Phase E 1 件目サマリ**: Get 200 継続、ネスト client-child 限定継続、逆方向不整合 通算 17 フィールド（M09 5 + M10 6 + M11 4 + M12 2）、Phase D 12 req + Phase E 4 req = **通算 16 req で Phase D/E 計 4 リソース完走**。
- **以前の完了**: M11 実 API E2E 実行で **Phase D（コア業務）3 件目 = 完走**: List FAIL（22 items, unmapped **4 件** = `cost / description / invoice_date / payment_date`）/ Get FAIL（**200 成功** = Phase D コア業務系 Get 提供が **3 件連続で確定**、unmapped 4 件 + `ProjectCostEntity` 8 フィールド中 **4 フィールド（半分）が逆方向不整合**）/ Search FAIL（22 items, unmapped 4 件, `ProjectID=0`＝非付与 で全件返却）。実 API キー構成（8 個）: `[cost, created_at, description, id, invoice_date, payment_date, project_id, updated_at]`。**親エンティティのネスト構造（`project:{...}`）は発現せず** → ネストパターンは M09/M10 で観測された `client:{id, name, name_disp, custom_no}` パターンが "client の子リソース特有" であることが確定（project の子である project_costs には適用されない）。既存 `ProjectCostEntity` は 8 フィールド中 **4 フィールド**（`Name / CostType / Amount / Memo`）が実 API に存在せず逆方向不整合（Memo 逆方向は **7 件連続**）。**概念モデル根本ズレの発見**: Entity は「労務費/資材費のカテゴリ集計」想定、実 API は「仕訳的 expense entry（`description`+`cost`+`invoice_date`+`payment_date`）」の個別支払い記録フラット構造。Unit 5/5 Green。403/429/リソース全体 403 は発生せず。実消費 **4 req**（見積 4 req、pin-point accuracy、上限 10 以内）。**Phase D 完走サマリ**: M09/M10/M11 全件で Get 200 成功、ネストは client-child 限定、逆方向不整合 計 15 フィールド（うち Memo が 3 件含む）、req 消費合計 12 req で完走。
- **次のアクション**: M13 (projects 厳格突合 + GetWithGroup 全 response_group、Phase E 2 件目、複雑度最高、~15 req) を着手
