...
| 作成日 | 2026-04-20 |
| 最終更新 | 2026-04-21 |
| ステータス | M22 完了（**Phase G 6 件目・invoices List/Get/Search 厳格突合**。`ListInvoicesRaw` / `GetInvoiceRaw` / `SearchInvoicesRaw` を `invoices.go` に追加。`e2e_invoices_test.go` 新規作成（WithPerPage(1) で大量データ対応）。旧 `TestE2E_Invoices_List` を `e2e_test.go` から削除。go build/vet/test 全 Green。実 API 未実行（~4 req 見込み）。） ※履歴は下記に保持 |
...
## Current Focus
- **マイルストーン**: M23 purchase_orders List/Get/Search + 厳格突合（Phase G 7 件目、~10 req）
- **直近の完了**: M22 **Phase G 6 件目 = invoices List/Get/Search 厳格突合**: `ListInvoicesRaw` / `GetInvoiceRaw` / `SearchInvoicesRaw` 追加、`e2e_invoices_test.go` 新規作成（WithPerPage(1) で大量データ対応）。旧 `TestE2E_Invoices_List` を e2e_test.go から削除し厳格突合版に一本化。go build/vet/test 全 Green。実消費 TBD（実 API 未実行、~4 req 見込み）。
- **以前の完了**: M21 **Phase G 5 件目 = receipts Get 厳格突合**: `GetReceiptRaw` 追加、`e2e_receipts_test.go` 新規作成（M17 helper 経由）。go build/vet/test 全 Green。実消費 TBD（実 API 未実行、~3 req 見込み）。
- **次のアクション**: M23 (purchase_orders List/Get/Search + 厳格突合、Phase G 7 件目、~10 req) を着手
