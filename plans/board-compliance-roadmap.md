...
| 作成日 | 2026-04-20 |
| 最終更新 | 2026-04-21 01:00 |
| ステータス | M13 完了（**Phase E 2 件目・複雑度最高、`Memo` 逆方向 9 件連続・name filter 無視 9 件連続で BOARD API 全般仕様最終確定**。List FAIL（2405 items, unmapped **20**）/ Get FAIL（**200 成功** = Phase D/E コア業務系 Get **5 件連続 200**、unmapped **62** = List 20 + Get 限定 42）/ Search FAIL（2405 items, unmapped 20, name filter 無視 **9 件連続**）/ GetWithGroup 6 subtest 全 FAIL（意図的）。Unit 6/6 Green。**最重要新発見**: delivery/invoice/receipt は `deliveries`/`invoices`/`receipts` という **複数形配列キー**で返却 → `*DocumentSummary` 単一ポインタ設計が根本的に誤り。estimate / order のみ単一オブジェクト（現行設計が partially 正しい）。`client_id=0`（ネスト `client.id` が正規）、`status=空`（`order_status`/`delivery_status` が代替）。実消費 11 req（見積 11、pin-point accuracy）） ※履歴は下記に保持 |
| ステータス履歴 | M12 完了（**Phase E 1 件目、Get > List 情報量差モデル新発見、`Memo` 逆方向 8 件連続で BOARD API 全般仕様最終確定**。List FAIL（299 items, unmapped **15**）/ Get FAIL（**200 成功 = Phase D/E コア業務系 Get 4 件連続 200**、unmapped **29** = List 15 + Get 限定 14）/ Search FAIL（299 items, unmapped 15, name filter 無視 **7 件連続**）。Unit 5/5 Green。`ClientEntity` は 6 フィールド中 **2 つ（Code/Memo）が逆方向不整合**、**既存 Entity の根本不足が M12 で最大規模（271cba3 の 3 倍規模）**に到達。**ネスト構造は発現せず** → M11 確定「ネストは client の子リソース特有」法則に沿う（clients 自身はフラット）。Get は List より 14 フィールド多い情報リッチ応答（**新 2 段階モデル**）。実消費 4 req（見積 5 以下、pin-point accuracy）） / M11 完了（**Phase D 完走、3 件連続 Get 200 確定**。List FAIL（22 items, unmapped **4** = `cost / description / invoice_date / payment_date`）/ Get FAIL（**200 成功 = Phase D コア業務系 Get 提供が 3 件連続確定**、unmapped 4 + 既存 4 フィールド逆方向不整合）/ Search FAIL（22 items, unmapped 4, `ProjectID=0` 非付与で全件返却）。Unit 5/5 Green。`ProjectCostEntity` は 8 フィールド中 **半分（4 つ）が逆方向不整合**（`Name/CostType/Amount/Memo` が実 API に不在）。**ネスト構造は発現せず** → ネストパターンは "client の子" 特有と確定。**概念モデルが根本ズレ**: Entity は「労務費/資材費の集計」想定、実 API は「仕訳的 expense entry（`description`+`cost`+`invoice_date`+`payment_date`）」。Memo 逆方向パターン **7 件連続**で全般仕様確定。実消費 4 req（pin-point accuracy）） |
...
## Current Focus
- **マイルストーン**: M14 vendor_branches 完走（Phase F 1 件目、payee_branches 実パス、~8 req）
- **直近の完了**: M13 実 API E2E 実行で **Phase E 2 件目 = projects 厳格突合 + GetWithGroup 全 6 response_group**: List FAIL（2405 items, unmapped **20**）/ Get FAIL（**200 成功** = Phase D/E コア業務系 Get **5 件連続 200 確定**、unmapped **62** = List 20 + Get 限定 42）/ Search FAIL（2405 items, unmapped 20, `name` filter 無視 **9 件連続**）/ GetWithGroup 6 subtest 全 FAIL（意図的）。**M13 最重要新発見**: ①delivery/invoice/receipt は `deliveries`/`invoices`/`receipts` という **複数形配列キー**で返却 → `*DocumentSummary` 単一ポインタ設計が根本的に誤り（estimate / order のみ単一オブジェクトで partially 正しい）、②`client_id=0`（List では `client` ネストオブジェクト内の `id` が正規キー、M09/M10 と同様）、③`status=空`（実 API は `order_status`/`delivery_status` で代替）、④`rg=all` 時に `project_costs` キーが出現（M17+ スコープ）。Unit 6/6 Green（U6 で GetWithGroup response_group query 検証を初導入）。403/429 は発生せず。実消費 **11 req**（見積 11、pin-point accuracy）。**`Memo` 逆方向 9 件連続・name filter 無視 9 件連続で BOARD API 全般仕様として最終確定**。
- **以前の完了**: M12 実 API E2E 実行で **Phase E 1 件目 = clients 厳格突合**: List FAIL（299 items, unmapped **15**）/ Get FAIL（**200 成功** = Phase D/E コア業務系 Get **4 件連続 200 確定**、unmapped **29** = List 15 + Get 限定 **14 フィールド**）/ Search FAIL（299 items, unmapped 15, `name` filter 無視 **7 件連続**）。**M12 新発見**: Get > List 情報量差 2 段階モデル、`ClientEntity` は 6 フィールド中 2 つ（Code/Memo）が逆方向不整合、ネスト構造は発現せず。Unit 5/5 Green。実消費 **4 req**。
- **次のアクション**: M14 (vendor_branches 完走、payee_branches 実パス、Phase F 1 件目、~8 req) を着手
...
#### M13: projects 厳格突合 + GetWithGroup 全 response_group 🟡（Phase E 2 件目・複雑度最高、List/Get/Search/GetWithGroup(×6) すべて意図的 Fail = List 20 未マップ / Get 62 未マップ / DocumentSummary 配列構造の根本的不一致）
...
- [x] 厳格フィールド突合で **List/Search 20 未マップ**（`client, contact, delivery_status, delivery_status_name, estimate_date, group_id, group_name, invoice_dates, management_no, order_status, order_status_name, project_no, project_type2_id/name, project_type3_id/name, project_type_id/name, tax, total, user`）
- [x] **Get 62 未マップ**（List 20 + Get 限定 42 フィールド：accounting_type* / archive_flg / client_branch / contract_* / cost_* / delivery_* / document_setting_* / hubspot / in_house_memo / invoice_* / lock_flg / monthly_* / ordered_date / payment_* / periodical_* / tags / to 等）
...
- E2E 結果: List FAIL（2405 items, 20 unmapped）/ Get FAIL（200、62 unmapped + 複数逆方向不整合）/ Search FAIL（2405 items, 20 unmapped, filter 無視）/ GetWithGroup 6/6 subtest FAIL（意図的、estimate/order は発現、delivery/invoice/receipt は配列構造不一致）
