...
| ステータス | M13 完了（**Phase E 2 件目・複雑度最高、`Memo` 逆方向 9 件連続・name filter 無視 9 件連続で BOARD API 全般仕様最終確定**。List FAIL（2405 items, unmapped **21**）/ Get FAIL（**200 成功** = Phase D/E コア業務系 Get **5 件連続 200**、unmapped **68** = List 21 + Get 限定 47）/ Search FAIL（2405 items, unmapped 21, name filter 無視 **9 件連続**）/ GetWithGroup 6 subtest 全 FAIL（意図的）。Unit 6/6 Green。**最重要新発見**: delivery/invoice/receipt は `deliveries`/`invoices`/`receipts` という **複数形配列キー**で返却 → `*DocumentSummary` 単一ポインタ設計が根本的に誤り。estimate / order のみ単一オブジェクト（現行設計が partially 正しい）。`client_id=0`（ネスト `client.id` が正規）、`status=空`（`order_status`/`delivery_status` が代替）。実消費 11 req（見積 11、pin-point accuracy）） ※履歴は下記に保持 |
