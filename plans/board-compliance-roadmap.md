...
| 作成日 | 2026-04-20 |
| 最終更新 | 2026-04-21 |
| ステータス | M23 完了（**Phase G 7 件目・purchase_orders List/Get/Search 厳格突合**。`ListPurchaseOrdersRaw` / `GetPurchaseOrderRaw` / `SearchPurchaseOrdersRaw` を `purchase_orders.go` に追加。`e2e_purchase_orders_test.go` 新規作成。旧 `TestE2E_PurchaseOrders_List` / `TestE2E_Payments_List` を `e2e_test.go` から削除。go build/vet/test 全 Green。実 API 未実行（~4 req 見込み）。） ※履歴は下記に保持 |
...
## Current Focus
- **マイルストーン**: M24 payments List/Get/Search + 厳格突合（Phase G 8 件目、~10 req）
- **直近の完了**: M23 **Phase G 7 件目 = purchase_orders List/Get/Search 厳格突合**: `ListPurchaseOrdersRaw` / `GetPurchaseOrderRaw` / `SearchPurchaseOrdersRaw` 追加、`e2e_purchase_orders_test.go` 新規作成。旧 `TestE2E_PurchaseOrders_List` / `TestE2E_Payments_List` を e2e_test.go から削除し厳格突合版に一本化。go build/vet/test 全 Green。実消費 TBD（実 API 未実行、~4 req 見込み）。
- **以前の完了**: M22 **Phase G 6 件目 = invoices List/Get/Search 厳格突合**: `ListInvoicesRaw` / `GetInvoiceRaw` / `SearchInvoicesRaw` 追加、`e2e_invoices_test.go` 新規作成（WithPerPage(1) で大量データ対応）。go build/vet/test 全 Green。実消費 TBD（実 API 未実行、~4 req 見込み）。
- **次のアクション**: M24 (payments List/Get/Search + 厳格突合、Phase G 8 件目・Phase G 完走、~10 req) を着手
