...
- [x] TestE2E_FindVendor_StrictEnrichment（vendors 0 件 → SKIP / pending re-verification）
- [x] TestE2E_FindVendor_ByName / ByText（vendors 0 件 → SKIP）
- [x] TestE2E_FindPurchaseOrder_ByID_Strict（purchase_orders 0 件 → SKIP）
- [x] TestE2E_FindPurchaseOrder_ByVendorName_Strict（vendors 0 件 → SKIP）
- [x] TestE2E_FindPurchaseOrder_ByProjectName_Strict（cache-warm SKIP、cascade-fetch リスク回避）
- [x] TestE2E_FindPurchaseOrder_ByText_Strict（purchase_orders 0 件 → SKIP）
- [x] TestE2E_FindPurchaseOrder_ByStatus_Strict（purchase_orders 0 件 → SKIP）
- [x] TestE2E_FindPayment_ByID_Strict（payments 0 件 → SKIP）
- [x] TestE2E_FindPayment_ByVendorName_Strict（vendors 0 件 → SKIP）
- [x] TestE2E_FindPayment_ByPurchaseOrderID_Strict（purchase_orders 0 件 → SKIP）
- [x] TestE2E_FindPayment_ByText_Strict（payments 0 件 → SKIP）
- [x] TestE2E_FindPayment_ByStatus_Strict（payments 0 件 → SKIP）
- **全 13 テスト SKIP（明示的・data-dependent）**: vendors/purchase_orders/payments 全 0 件
- **M25 同型 enrichment バグ**: VendorBranchRepository/VendorContactRepository にも潜在バグがあるが、データ 0 件のため表面化せず。Pending Re-verification に追加
- 実消費: ~5 req（ListVendorsPage / ListPurchaseOrdersPage / ListPaymentsPage 各 1 req × SKIP確認）
- E2E 結果: 全 13 テスト SKIP（PASS 相当）/ go build/vet/test 全 Green
- 詳細: plans/board-compliance-m30-find-vendor-side.md

#### M31: FindUser / FindGroup 厳格突合 ✅（Phase H 7 件目）
- [x] TestE2E_FindUser_ByID_Strict: PASS（DisplayName フォールバック Name="" → "立花 拓也" 確認）
- [x] TestE2E_FindUser_ByName_StrictAddon: SKIP（BOARD API name filter 無視で 0 件）
- [x] TestE2E_FindGroup_ByID_Strict: SKIP（groups 0 件 Pending Re-verification）
- [x] TestE2E_FindGroup_ByName: SKIP（groups 0 件 Pending Re-verification）
- [x] 既存 TestE2E_FindUser_ByName のパニックバグ修正（0 件時 results[0] アクセス）
...
| M30 | vendors (find) | FindVendor_StrictEnrichment | vendors 0 件。データ投入後 M25 同型 enrichment バグ（VendorBranchRepository/VendorContactRepository.Search in-memory 誤動作）を検出する可能性あり | データ投入後: `go test -tags e2e -v -count=1 -run TestE2E_FindVendor_StrictEnrichment ./internal/service/find/` |
| M30 | purchase_orders (find) | FindPurchaseOrder_ByID_Strict / ByText_Strict / ByStatus_Strict | purchase_orders 0 件 | データ投入後: `go test -tags e2e -v -count=1 -run TestE2E_FindPurchaseOrder ./internal/service/find/` |
| M30 | payments (find) | FindPayment_ByID_Strict / ByPurchaseOrderID_Strict / ByText_Strict / ByStatus_Strict | payments 0 件 | データ投入後: `go test -tags e2e -v -count=1 -run TestE2E_FindPayment ./internal/service/find/` |
...
| 2026-04-22 18:45 | M44 実装・Phase K M44 完了 | ProjectEntity を 13 フィールドから 72 フィールドへ全面再設計（Breaking / 最大規模、Phase K）。**削除**: Status/Code/Memo/Delivery・Invoice・Receipt 単数ポインタ（5 フィールド）。**追加**: ProjectNo, ManagementNo, Client *ClientRef, Contact *ContactRef, User *UserRef, Total *string, Tax *string, OrderStatus/DeliveryStatus int, OrderStatusName/DeliveryStatusName string, Hubspot *HubspotRef, CostTotal/CostTax int, ContractStartDate/ContractEndDate *string, Tags []string, InHouseMemo *string, Deliveries/Invoices/Receipts []DocumentSummary, ProjectCosts json.RawMessage 他 68 フィールド。**nested 型 5 件新規追加**: UserRef/ContactRef/ClientBranchRef/CompanyBranchRef/HubspotRef。**DocumentSummary 拡張**: 17 フィールド追加（共通 5 + type-specific 12、Details []DocumentDetailEntity 再利用）。**Unit test 12 件追加**: U1-U12（新 JSON スキーマ・DocumentSummary・nested 型・List/Get/Search パラメータ検証）。**downstream 修正 9 ファイル**: repository/projects.go, find_project.go, text_match.go, find_estimate/order/delivery/receipt/invoice.go, helpers_test.go, repository/projects_test.go。**E2E smoke PASS**: List/Get/Search/GetWithGroup 全 6 variants 未マップ 0（11 req、plan cap 15 以内）。go build/vet/test 全 Green。commit: 88139f9, b20c138, 0d1f652, f1c2eee, 2c5e254。詳細: plans/board-phase-k-m44-project-entity.md。 |
| 2026-04-22 19:10 | M45 実装・Phase K M45 完了 | ProjectCostEntity を概念モデルから全面再設計（Breaking）。**削除**: Name/CostType/Amount/Memo（「カテゴリ集計」モデル残滓の 4 フィールド）。**追加**: Description string / Cost int / InvoiceDate *string / PaymentDate *string（「個別支払い記録」モデルへ意味変化）。**M44 連携**: ProjectEntity.ProjectCosts を json.RawMessage から []ProjectCostEntity へ変換。**Unit test 3 件追加**: U1（8 フィールド全検証）/ U2（null 日付ポインタ nil 確認）/ U3（project_id クエリエンコード確認）。**downstream 修正 4 ファイル**: boardapi/client_test.go, project_costs_test.go, e2e_project_costs_test.go, repository/project_costs_test.go。**E2E smoke PASS**: List 22 件 / Get 1 件 / Search 22 件 未マップ 0（3 req）。M44 TestE2E_Projects_GetWithGroup 全 6 variants 引き続き PASS。go build/vet/test 全 Green。commit: 8ce558f, 38f656c。詳細: plans/board-phase-k-m45-project-cost-entity.md。 |
