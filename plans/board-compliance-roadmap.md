...
| 作成日 | 2026-04-20 |
| 最終更新 | 2026-04-21 02:05 |
| ステータス | M17 完了（**Phase G 1 件目・documentID discovery helper 確立**。`findAnyDocumentID(t, client, docType)` を `e2e_helpers_test.go` に追加。Unit テスト（httptest ベース: estimate found + delivery found + unknown docType doc）Green。実 API smoke test（estimate 1 docType）PASS: projectID=95944469 documentID=105287235 発見。実消費 **2 req**（見積 5）。helper シグネチャ: `(t *testing.T, client *boardapi.Client, docType string) (projectID, documentID int)`。probe struct 方式で全 5 docType を統一処理（`ProjectEntity` の delivery/invoice/receipt JSON タグ単数形ミスマッチを回避）。） ※履歴は下記に保持 |
...
## Current Focus
- **マイルストーン**: M17 documentID discovery helper 確立（Phase G 開始、~5 req）
- **直近の完了**: M16 **Phase F 3 件目・Phase F 完走 = vendors（payees 実パス）**: List PASS（0 items）/ Get SKIP（0 items = data-dependent skip）/ Search PASS（0 items）。Unit 5/5 Green。実消費 **3 req**（見積 5）。**Phase F 完走サマリ**: M14 vendor_branches + M15 vendor_contacts + M16 vendors の全 3 件でデータ 0 件（当該アカウントはベンダー系リソース全て空）。実パス命名不一致（`payees`/`payee_branches`/`payee_contacts`）を Unit テストパスアサーションで確認済み。Phase F 合計 req: **9 req**（各 3 req × 3）。
- **以前の完了**: M15 **Phase F 2 件目 = vendor_contacts（payee_contacts 実パス）**: List PASS（0 items）/ Get SKIP（0 items = data-dependent skip）/ Search PASS（0 items）。Unit 5/5 Green。実消費 **3 req**（見積 8）。
- **次のアクション**: M17 (documentID discovery helper 確立、Phase G 開始、~5 req) を着手
...
- 見積: ~5 req / 実績: **2 req**（List 1 + GetProjectWithGroupRaw 1）
...
| 2026-04-21 02:05 | M17 実装・検証・Phase G 開始 | `findAnyDocumentID(t, client, docType)` を `e2e_helpers_test.go` に追加。シグネチャ: `(t, client, docType) → (projectID, documentID int)`。probe struct 方式（`GetProjectWithGroupRaw` + anonymous struct）で全 5 docType を統一処理し、`ProjectEntity` の delivery/invoice/receipt JSON タグ単数形ミスマッチを回避。estimate/order は単一オブジェクト、delivery/invoice/receipt は複数形配列の先頭を返す。上限 `maxDiscoveryProjects=3` で rate limit 配慮。Unit テスト（httptest ベース）2 本 Green。実 API smoke test（estimate）: projectID=95944469 documentID=105287235、**2 req**（見積 5）。M18 以降の estimates/orders/deliveries/receipts/invoices Get E2E で即再利用可能。|
