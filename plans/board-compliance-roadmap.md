...
| ステータス | M18 完了（**Phase G 2 件目・estimates Get 厳格突合**。`GetEstimateRaw` を `estimates.go` に追加。`e2e_estimates_test.go` 新規作成（`TestE2E_Estimates_Get` 1 本、M17 helper 経由）。旧 `TestE2E_Estimates_GetByDocumentID` を削除して一本化。go build/vet/test 全 Green。実 API 未実行（~3 req 見込み）。） ※履歴は下記に保持 |
