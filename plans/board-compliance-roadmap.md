...
| 作成日 | 2026-04-20 |
| 最終更新 | 2026-04-21 |
| ステータス | M21 完了（**Phase G 5 件目・receipts Get 厳格突合**。`GetReceiptRaw` を `receipts.go` に追加。`e2e_receipts_test.go` 新規作成（`TestE2E_Receipts_Get` 1 本、M17 helper 経由）。go build/vet/test 全 Green。実 API 未実行（~3 req 見込み）。） ※履歴は下記に保持 |
...
## Current Focus
- **マイルストーン**: M22 invoices Get/Search + 厳格突合（Phase G 6 件目、~20 req）
- **直近の完了**: M21 **Phase G 5 件目 = receipts Get 厳格突合**: `GetReceiptRaw` 追加、`e2e_receipts_test.go` 新規作成（M17 helper 経由）。go build/vet/test 全 Green。実消費 TBD（実 API 未実行、~3 req 見込み）。
- **以前の完了**: M20 **Phase G 4 件目 = deliveries Get 厳格突合**: `GetDeliveryRaw` 追加、`e2e_deliveries_test.go` 新規作成（M17 helper 経由）。go build/vet/test 全 Green。実消費 TBD（実 API 未実行、~3 req 見込み）。
- **次のアクション**: M22 (invoices Get/Search + 厳格突合、Phase G 6 件目、~20 req) を着手
