...
## Current Focus
- **マイルストーン**: N08（MCP tools 刷新、input schema 整理）— N07c 完走済
- **直近の完了**: N07c 完走（2026-04-27）。CLI/MCP の name → ID 解決を配線（`Service.ResolveClientByName` / `ResolveVendorByName` 新設、find package に集約）。重複ヒット時の曖昧性 error + 候補列挙（最大 5 件）、silent take-first 禁止。Document 4 種は fanout 検索のため disambiguate なし（API reference / CHANGELOG で明記）。構造的未対応フラグ（Document `--status` / Payment `--project-name`）と将来拡張フラグ（Invoice/PO `--project-name`、Payment `--purchase-order-id`）の最終エラー文言を確定。enrichment non-fatal の breaking change を CHANGELOG / api-reference に告知。go test -race ./... pass / go vet pass / gofmt 差分なし
- **次のアクション**: N08 — MCP tools 刷新（input schema 整理、未対応フラグの schema 削除、tool 説明文の LLM 向け磨き）
