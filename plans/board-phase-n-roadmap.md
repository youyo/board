...
| 作成日 | 2026-04-24 |
| 最終更新 | 2026-04-27（Phase N 全 10 マイルストーン完走、v0.7.0 タグ作成済 / push 待機） |
| ステータス | **Phase N 全完了**（N01-N10 完走）、v0.7.0 タグ作成済 / push 待機 |
...
## Current Focus
- **マイルストーン**: **Phase N 全完了**（N01-N10、2026-04-25 〜 2026-04-27）
- **直近の完了**: N09 完走（2026-04-27）。E2E テストを実 API 感応する代表ケースのみへ再構築。Service 層 41 ケース (T01-T41) + MCP in-process handler 経由 5 ケース (T42-T46) = **計 46 ケース**（旧 193 から 78% 削減）。SKIP 統一テンプレート 4 種（`[SKIP:no-creds|no-data|cache-warm|rate-limit] msg`）で grep 集計可能化。per-batch 実行（rate-limit 配慮）+ CI 非実行（ローカル開発者手動）の運用に移行。構造的不可フラグ（D1）/ 契約上未実装フラグ（D4）の handler reject、disambiguate vs fanout の挙動差を T42-T46 で実機検証可能。Payment.Project = nil 仮説 (D1) の E2E 再確認は T37 で実装。`go vet ./...` / `go vet -tags e2e ./...` / `go test ./...` / `go test -tags e2e -run '^$'` 全 pass、credentials 無し環境では全 e2e ケースが `[SKIP:no-creds]` 動作確認済。README "Testing" / docs/specs §39 / docs/adr/ADR-001 / CHANGELOG 更新。
- **N10 完走（2026-04-27）**: CHANGELOG.md の Unreleased を `[0.7.0] - 2026-04-27` に確定（Breaking まとめ + Added / Changed / Removed / Tests / Internal / Migration の 7 セクション構造化）、`docs/migration/v0.7.0.md` を新規作成（v0.6.0 → v0.7.0 マイグレーションガイド 8 章構成）、README.md / README_ja.md を find 11 tool / 仕様サマリー / migration link に最終化、`.claude-plugin/plugin.json` を 0.7.0 に bump、ADR-001 Implementation Status に N10 追記（N06 deferred review trigger ≈ 2026-05-11 / Payment.Project=nil 本番再評価 / D6 cache miss 実測待ちを明記）。production code 0 行変更（docs / chore のみ）。`go test ./...` / `go vet ./...` / `gofmt -s -w .` / `go build ./...` 全 pass。`git tag v0.7.0` をローカル作成済（push は待機）。
- **次のアクション**: ユーザー承認後に `git push` + `git push origin v0.7.0` → GoReleaser 経由で GitHub Releases / Homebrew tap 配信。N06 deferred review は 2026-05-11 頃に実施（呼び出し実績 50% 以下なら ADR-002 起票）。

## Phase N 全体総括

| 項目 | 数値 |
|------|------|
| マイルストーン数 | 10（N01-N10、N07 は a/b/c 三分割で実質 12） |
| 期間 | 2026-04-24 〜 2026-04-27（4 日間、計画 5-7 週見積から大幅短縮） |
| コミット数 | 30+ |
| Find メソッド数 | 12 → 11（`FindGroup` 削除） |
| MCP tool 数 | 12 → 11（`find_groups` 削除） |
| E2E ケース数 | 193 → 46（78% 削減） |
| Breaking changes | 7 件（find_groups / enrichment non-fatal / disambiguate / status narrowing / MCP schema 削除 / Document fanout / E2E 構造） |
| ADR 起票 | ADR-001 Accepted（B 採択、ゼロベース再設計） |
| 確立パターン | reverseMapper / non-fatal enrichment / filterByStatuses[T] / disambiguate vs fanout / SKIP テンプレ 4 種 / per-batch E2E / recordingHandler |
| 成果物 | v0.7.0 タグ（push 待機） |
...
- **N07c**: CLI/MCP の name → ID 解決配線 + 構造的未対応フラグ整理 — ✅ 完了（2026-04-27）。`Service.ResolveClientByName` / `ResolveVendorByName` を find package に新設、9 unit test。CLI 4 種 (project/invoice/purchase_order/payment) で resolver 経路を配線、Document 4 種で `--client-name` / `--project-name` を service に pass（fanout）、`--status` / 構造的未対応フラグの最終エラー文言を確定。MCP tools.go も同様。enrichment non-fatal の breaking change を CHANGELOG / api-reference / ADR-001 に告知
- **N08**: MCP tools schema 刷新（11 tool、LLM 向け description 磨き、構造的不可フラグの schema 削除）— ✅ 完了（2026-04-27）。`internal/mcpserver/tools.go` 全面書き直し、新 description helper 4 種、Document 4 種から `status` schema 削除、disambiguate vs fanout 挙動差を description で明示、`find_projects.status` narrowing 必須を明記、契約上未実装 3 件に `(NOT YET SUPPORTED)` 警告。MCP の refresh は意図的に未公開（D6）。tools_test に 4 新規 test 追加。CHANGELOG / docs/specs §23 / docs/api-reference / ADR-001 / 埋込 docs 同期を更新
- **N09**: E2E テスト再構築（33-41 代表ケース、実 API 感応のみ、SKIP 理由は明示ログ）— ✅ 完了（2026-04-27）。Service 層 41 ケース + MCP handler 経由 5 ケース = 計 46 ケース（旧 193 から 78% 削減）。SKIP 統一テンプレ 4 種、per-batch 実行設計、CI 非実行運用に移行。N02 §8.3 の上限 41 は Service 層に限定したものとして再解釈（タスク指示の MCP 経由要請に応じて T42-T46 を別カテゴリで追加）。`internal/service/find/e2e_*_test.go` (8 ファイル) + `internal/mcpserver/e2e_handler_test.go` 新規。production code 0 行変更
- **N10**: v0.7.0 リリース準備 — ✅ 完了（2026-04-27）。CHANGELOG v0.7.0 確定、`docs/migration/v0.7.0.md` 新規（8 章）、README/README_ja 最終化、`.claude-plugin/plugin.json` を 0.7.0 bump、ADR-001 N10 追記、`git tag v0.7.0` ローカル作成（push 待機）。production code 0 行変更
...
| 2026-04-27 | 完了 | N07c 完走（self-execute、advisor 2 ラウンド反映）。`Service.ResolveClientByName` / `ResolveVendorByName` を find package に新設（D1: receiver メソッド方式、CLI/MCP 共通利用）、+9 unit test。**重複ヒット時の曖昧性 error + 候補上限 5 件列挙**（D2、silent take-first 禁止）。CLI 4 種 (project/invoice/purchase_order/payment) で resolver 経路配線、Document 4 種で `--client-name`/`--project-name` を service に pass-through（fanout 検索、disambiguate なし）。**構造的未対応 (does not support) 2 件 + 将来拡張 (not yet supported) 3 件**の最終エラー文言を確定。enrichment non-fatal の breaking change を CHANGELOG / api-reference / ADR-001 に告知。CLI find_test に `TestFindCmd_RejectFlagErrorMessages` 追加（7 ケース、`--id` 併用で resolver 経路前段を検証）。go test -race ./... pass、go vet pass、gofmt 差分なし。次は N08（MCP tools schema 刷新） |
| 2026-04-27 | 完了 | N10 完走（self-execute、advisor 1 ラウンド反映）。CHANGELOG.md の Unreleased を `[0.7.0] - 2026-04-27` セクションに確定（Breaking changes まとめ + Added / Changed / Changed (Breaking) / Removed / Tests / Internal / Migration の 7 セクション構造化）。`docs/migration/v0.7.0.md` 新規作成（v0.6.0 → v0.7.0 マイグレーションガイド、8 章構成: 概要 / find_groups 削除 / enrichment non-fatal / name → ID 重複ヒット / Status narrowing / find2 → find rename / Document fanout / MCP schema 刷新 / E2E 構造変更）。README.md / README_ja.md を find 11 tool（`find_group` 削除）+ 仕様サマリー（disambiguate vs fanout / status narrowing / non-fatal enrichment）+ migration link に最終化。`.claude-plugin/plugin.json` を 0.6.0 → 0.7.0 に bump。ADR-001 Implementation Status に N10 追記（N06 deferred review trigger ≈ 2026-05-11 を明記）。production code 0 行変更（docs / chore のみ）。`go test ./...` / `go vet ./...` / `gofmt -s -w .` / `go build ./...` 全 pass。`git tag v0.7.0` をローカル作成（push 待機）。Phase N 全 10 マイルストーン完走（N01-N10、4 日間、コミット 30+、find 12 → 11、E2E 193 → 46）。 |
| 2026-04-27 | 完了 | N09 完走（self-execute、advisor 2 ラウンド反映）。E2E テストを実 API 感応する代表ケースのみへ再構築（旧 193 → Service 41 + MCP handler 5 = 46、78% 削減）。SKIP 統一テンプレ 4 種（`[SKIP:no-creds\|no-data\|cache-warm\|rate-limit]`）、per-batch 実行設計、CI 非実行運用。Service 層 E2E 8 ファイル新規（`e2e_helpers_test.go` / `e2e_find_client_test.go` / `e2e_find_vendor_test.go` / `e2e_find_user_test.go` / `e2e_find_project_test.go` / `e2e_find_documents_test.go` / `e2e_find_invoice_test.go` / `e2e_find_purchase_order_test.go` / `e2e_find_payment_test.go`）。MCP in-process handler 経由 E2E 1 ファイル新規（`internal/mcpserver/e2e_handler_test.go`、T42-T46: status reject / not-yet-supported / disambiguate / fanout 挙動差を実機検証）。production code 0 行変更（テストのみ）。Payment.Project = nil 仮説 (D1) の E2E 再確認 (T37)。MCP refresh 未公開 (D6) の cache miss 観測ポイント T45/T46 を確保。`go vet ./...` / `go vet -tags e2e ./...` / `go test ./...` / `go test -tags e2e -run '^$' ./internal/service/find/ ./internal/mcpserver/` 全 pass。credentials 無しで全 e2e ケースが `[SKIP:no-creds]` で skip されること確認済。docs/specs §39 / README "Testing" / ADR-001 Implementation Status / CHANGELOG 更新。次は N10（v0.7.0 リリース準備） |
| 2026-04-27 | 完了 | N08 完走（self-execute、advisor 2 ラウンド反映）。MCP 11 tool schema / description を LLM 向けに刷新。**Document 4 種の `status` プロパティを構造的不可（D1, never-implementable）として schema 削除**（primary defense として handler reject 残置、mark3labs/mcp-go は server-level schema validation 強制せず）、**契約上未実装フラグ（D4, contingently unimplemented）は schema 残置 + property description で `(NOT YET SUPPORTED)` 警告**（`find_invoices.project_name` / `find_purchase_orders.project_name` / `find_payments.purchase_order_id`）。**`find_projects.status` の narrowing 必須（N05）を description に明記**（advisor R1 反映、status-only クエリは reject）。disambiguate 系 4 tool（projects/invoices/purchase_orders/payments）と fanout 系 4 tool（estimates/orders/deliveries/receipts）の挙動差を description で峻別。新 description helper 4 種（`disambiguateNameDesc` / `fanoutNameDesc` / `notYetSupportedDesc` / `limitDesc`）で生成統一。**MCP の refresh / force_refresh は意図的に未公開（D6）**: handler は resolver に空 `ReadOptions{}` を渡し cache hit 前提。LLM tool 設計を複雑化させない方針で ADR-001 / api-reference §MCP に明記、N09+ で再評価。tools_test に 4 新規 test 追加（disambiguation / fanout / not-yet-supported / narrowing 言及確認、+containsAny helper）、Document 4 種の `status` プロパティ非存在 assert 追加。CHANGELOG（Breaking + Changed）/ docs/specs §23.2 §23.5 §23.8 / docs/api-reference / ADR-001 Implementation Status / 埋込 docs 同期更新。go test -race ./... 全 14 パッケージ pass、go vet pass、gofmt 差分なし。次は N09（E2E テスト再構築） |
