...
## Current Focus
- **マイルストーン**: N09（E2E テスト再構築、33-41 代表ケース）— N08 完走済
- **直近の完了**: N08 完走（2026-04-27）。MCP 11 tool の schema / description を LLM 向けに刷新。Document 4 種の `status` プロパティを構造的不可（D1）として schema 削除（primary defense として handler reject 残置）、契約上未実装フラグ（D4: `find_invoices.project_name` / `find_purchase_orders.project_name` / `find_payments.purchase_order_id`）は schema 残置 + property description で `(NOT YET SUPPORTED)` 警告。`find_projects.status` の narrowing 必須（N05）を description に明記。disambiguate 系 4 tool と fanout 系 4 tool の挙動差を description で峻別。新 helper（`disambiguateNameDesc` / `fanoutNameDesc` / `notYetSupportedDesc` / `limitDesc`）で description 生成を統一。MCP の refresh / force_refresh は意図的に未公開（D6、ADR-001 / api-reference に明記）。tools_test に 4 新規 test 追加（disambiguation/fanout/not-yet-supported/narrowing 言及確認）。go test -race ./... pass / go vet pass / gofmt 差分なし
- **次のアクション**: N09 — E2E テスト再構築（33-41 代表ケース、実 API 感応のみ、SKIP 理由は明示ログ）
