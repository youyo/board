...
// accountingTypeListFlagsFromCmd reads the Ransack-style filter flags and returns a
// AccountingTypeListOptions. All flags are optional; any flag left at its zero value
// is omitted from the outgoing request.
func accountingTypeListFlagsFromCmd(cmd *cobra.Command) boardapi.AccountingTypeListOptions {
	...
	return boardapi.AccountingTypeListOptions{
		NameCont:          nameCont,
...
func newAPIAccountingTypesListCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List accounting types (optionally filtered by Ransack-style query params)",
		Long: `List accounting types. Filters are forwarded to the BOARD API as Ransack-style
...
--no-show-meta to omit it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			filter := accountingTypeListFlagsFromCmd(cmd)
			result, err := svc.ListAccountingTypes(cmd.Context(), readOpts, filter)
			if err != nil {
				return err
			}
	...
	cmd.Flags().String("name-cont", "", "Filter by accounting type name (Ransack name_cont, partial match)")
	...
	cmd.Flags().Bool("include-archive-flg", false, "Include archived accounting types (send include_archive_flg=1)")
