...
// groupListFlagsFromCmd reads the Ransack-style filter flags and returns a
// GroupListOptions. All flags are optional; any flag left at its zero value
// is omitted from the outgoing request.
func groupListFlagsFromCmd(cmd *cobra.Command) boardapi.GroupListOptions {
	...
	return boardapi.GroupListOptions{
		NameCont:          nameCont,
...
func newAPIGroupsListCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "list",
		Short: "List groups (optionally filtered by Ransack-style query params)",
		Long: `List groups. Filters are forwarded to the BOARD API as Ransack-style
...
--no-show-meta to omit it.`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			filter := groupListFlagsFromCmd(cmd)
			result, err := svc.ListGroups(cmd.Context(), readOpts, filter)
			if err != nil {
				return err
			}
	...
	cmd.Flags().String("name-cont", "", "Filter by group name (Ransack name_cont, partial match)")
	...
	cmd.Flags().Bool("include-archive-flg", false, "Include archived groups (send include_archive_flg=1)")
