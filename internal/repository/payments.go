...
// PaymentRepository は payments リソースのキャッシュ → リフレッシュ → API フォールバックを管理する。
...
// NewPaymentRepository は PaymentRepository を生成する。
...
// Search はパラメータでフィルタした支払をキャッシュから返す。
...
// filterPayments はインメモリフィルタリングを行う。
