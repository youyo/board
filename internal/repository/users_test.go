...
var sampleUsers = []boardapi.UserEntity{
	{ID: 1, Name: "田中太郎", Email: "tanaka@example.com", UpdatedAt: "2026-01-01T00:00:00Z"},
	{ID: 2, Name: "鈴木花子", Email: "suzuki@example.com", UpdatedAt: "2026-01-02T00:00:00Z"},
	{ID: 3, Name: "佐藤次郎", Email: "sato@example.com", UpdatedAt: "2026-01-03T00:00:00Z"},
}

// T_USR01: List - キャッシュあり → キャッシュのデータを返す
...
// T_USR02: List - キャッシュなし（初回）→ ForceRefresh 後データを返す
...
// T_USR03: GetByID - キャッシュヒット → キャッシュから返す
...
// T_USR04: GetByID - キャッシュミス、API 成功 → API 取得して返す
func TestUserRepository_GetByID_CacheMiss_APISuccess(t *testing.T) {
	db := newTestDB(t)
	markSynced(t, db, "users")

	target := boardapi.UserEntity{ID: 99, Name: "テストユーザー", Email: "test@example.com", UpdatedAt: "2026-01-01T00:00:00Z"}
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
...
// T_USR05: GetByID - キャッシュミス、API エラー → error を返す
...
// T_USR06: Search - Name フィルタ → 一致するものを返す
func TestUserRepository_Search_NameFilter(t *testing.T) {
	...
	repo := makeUserRepo(t, db, apiClient, false)
	got, err := repo.Search(context.Background(), boardapi.UserSearchParams{Name: "田中太郎"}, repository.ReadOptions{})
	if err != nil {
...
// T_USR07: Search - Email フィルタ → 一致するものを返す
func TestUserRepository_Search_EmailFilter(t *testing.T) {
	...
	repo := makeUserRepo(t, db, apiClient, false)
	got, err := repo.Search(context.Background(), boardapi.UserSearchParams{Email: "suzuki@example.com"}, repository.ReadOptions{})
	if err != nil {
		t.Fatalf("Search: %v", err)
	}
...
func TestUserRepository_Search_NoFilter(t *testing.T) {
	...
	repo := makeUserRepo(t, db, apiClient, false)
	got, err := repo.Search(context.Background(), boardapi.UserSearchParams{}, repository.ReadOptions{})
	if err != nil {
		t.Fatalf("Search: %v", err)
	}
	if len(got) != len(sampleUsers) {
