...
var sampleReceipts = []boardapi.ReceiptEntity{
	{ID: 1, ClientID: 10, ProjectID: 100, Title: "領収A", Status: "draft", UpdatedAt: "2026-01-01T00:00:00Z"},
	{ID: 2, ClientID: 10, ProjectID: 101, Title: "領収B", Status: "issued", UpdatedAt: "2026-01-02T00:00:00Z"},
	{ID: 3, ClientID: 20, ProjectID: 102, Title: "領収C", Status: "issued", UpdatedAt: "2026-01-03T00:00:00Z"},
}

// T_REC01: List - キャッシュあり → キャッシュのデータを返す
...
// T_REC02: List - キャッシュなし（初回）→ ForceRefresh 後データを返す
...
// T_REC03: GetByID - キャッシュヒット → キャッシュから返す
...
// T_REC04: GetByID - キャッシュミス、API 成功 → API 取得して返す
func TestReceiptRepository_GetByID_CacheMiss_APISuccess(t *testing.T) {
	db := newTestDB(t)
	markSynced(t, db, "receipts")

	target := boardapi.ReceiptEntity{ID: 99, Title: "テスト領収", UpdatedAt: "2026-01-01T00:00:00Z"}
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
...
// T_REC05: GetByID - キャッシュミス、API エラー → error を返す
...
// T_REC06: Search - ClientID フィルタ → 一致するものを返す
...
// T_REC07: Search - Status フィルタ → 一致するものを返す
