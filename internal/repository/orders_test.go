...
var sampleOrders = []boardapi.OrderEntity{
	{ID: 1, ClientID: 10, ProjectID: 100, Title: "受注A", Status: "draft", UpdatedAt: "2026-01-01T00:00:00Z"},
	{ID: 2, ClientID: 10, ProjectID: 101, Title: "受注B", Status: "confirmed", UpdatedAt: "2026-01-02T00:00:00Z"},
	{ID: 3, ClientID: 20, ProjectID: 102, Title: "受注C", Status: "completed", UpdatedAt: "2026-01-03T00:00:00Z"},
}

// T_ORD01: List - キャッシュあり → キャッシュのデータを返す
...
// T_ORD02: List - キャッシュなし（初回）→ ForceRefresh 後データを返す
...
// T_ORD03: GetByID - キャッシュヒット → キャッシュから返す
...
// T_ORD04: GetByID - キャッシュミス、API 成功 → API 取得して返す
func TestOrderRepository_GetByID_CacheMiss_APISuccess(t *testing.T) {
	db := newTestDB(t)
	markSynced(t, db, "orders")

	target := boardapi.OrderEntity{ID: 99, Title: "テスト受注", UpdatedAt: "2026-01-01T00:00:00Z"}
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
...
// T_ORD05: GetByID - キャッシュミス、API エラー → error を返す
...
// T_ORD06: Search - ClientID フィルタ → 一致するものを返す
...
// T_ORD07: Search - Status フィルタ → 一致するものを返す
