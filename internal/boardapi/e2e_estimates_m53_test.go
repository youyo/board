...
// E2E tests for estimates M53: GetEstimate → *ItemResult[EstimateEntity].
//
// Scope (M53, board-phase-l): Phase L 刷新検証。
// GetEstimate の戻り値が *ItemResult[EstimateEntity] になったことを実 API で確認。
// ItemResult.Meta（parseItemMeta 経由）が rate limit 等のヘッダーを保持することを検証。
//
// 既存 e2e_estimates_test.go（M18/M35）との関係:
...
//	    -run TestE2E_Estimates_M53 ./internal/boardapi/
...
// TestE2E_Estimates_M53_GetReturnsItemResult は GetEstimate が *ItemResult を返し、
// 実 API レスポンスから ItemMeta（rate limit 等）が parseItemMeta 経由で取得できることを確認。
func TestE2E_Estimates_M53_GetReturnsItemResult(t *testing.T) {
	client := newE2EClient(t)
	ctx := context.Background()

	// Discover an estimate ID via M17 helper
	...
	// E1: GetEstimate が *ItemResult[EstimateEntity] を返すことを確認
	result, err := client.GetEstimate(ctx, docID)
	if err != nil {
		t.Fatalf("GetEstimate(%d): %v", docID, err)
	}
	if result == nil {
	...
	if result.Item.ID != docID {
		t.Errorf("GetEstimate: Item.ID = %d, want %d", result.Item.ID, docID)
	}
	t.Logf("E1 GetEstimate: id=%d total=%s details_count=%d", result.Item.ID, result.Item.Total, len(result.Item.Details))

	time.Sleep(400 * time.Millisecond)

	// E2: GetEstimateRaw が ([]byte, http.Header, error) を返し、ヘッダーが非 nil であることを確認
	raw, headers, err := client.GetEstimateRaw(ctx, docID)
	if err != nil {
		t.Fatalf("GetEstimateRaw(%d): %v", docID, err)
	}
	if len(raw) == 0 {
		t.Error("GetEstimateRaw: body is empty")
	}
	if headers == nil {
		t.Error("GetEstimateRaw: headers is nil")
	}
	// ヘッダー名のログ（M50 §E10 相当: 実測値確認）
	t.Logf("E2 GetEstimateRaw headers: Content-Type=%s ETag=%s X-Ratelimit-Remaining=%s X-RateLimit-Remaining=%s",
	...
	// 注意: GetEstimate は boardapi.Client を直接呼ぶため Headers は含まれる。
...
// TestE2E_Estimates_M53_ItemMetaParsed は ItemResult.Meta が非ゼロ値になることを確認（boardapi 直呼び）。
// parseItemMeta が X-Ratelimit-* ヘッダーを正しく取得できれば RateLimitLimit > 0 になるはず。
func TestE2E_Estimates_M53_ItemMetaParsed(t *testing.T) {
	client := newE2EClient(t)
	ctx := context.Background()

	_, docID := findAnyDocumentID(t, client, "estimate")
	requirePositiveID(t, docID, "findAnyDocumentID.documentID")
	...
	// boardapi.Client.GetEstimate は DoWithRetryFull でヘッダーを取得し parseItemMeta を適用。
