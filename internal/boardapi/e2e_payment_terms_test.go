...
// E2E tests for /v1/payment_terms against the real BOARD API.
//
// Scope (M04, board-compliance roadmap):
//   - List  (TestE2E_PaymentTerms_List)    : 1 req expected
//   - Get   (TestE2E_PaymentTerms_Get)     : 2 req expected (list to discover id + get)
//   - Search(TestE2E_PaymentTerms_Search)  : 1 req expected
//
...
// (gitignored) and runs testhelper.StrictFieldDiff against PaymentTermEntity
// to detect unmapped keys.
//
// Usage (single-shot):
//
//	BOARD_API_KEY=... BOARD_API_TOKEN=... go test -tags e2e -v -count=1 \
//	    -run TestE2E_PaymentTerms_List ./internal/boardapi/
...
// TestE2E_PaymentTerms_List exercises GET /v1/payment_terms and verifies
// that every JSON key returned by the BOARD API is mapped on PaymentTermEntity.
func TestE2E_PaymentTerms_List(t *testing.T) {
	...
	dumpJSON(t, "payment_terms", 0, raw)

	if diff := testhelper.StrictFieldDiff(t, raw, &[]boardapi.PaymentTermEntity{}); len(diff) > 0 {
		t.Errorf("payment_terms list unmapped fields: %v", diff)
	}

	// Parse minimally to log the observed count for traceability.
	var items []boardapi.PaymentTermEntity
	if err := json.Unmarshal(raw, &items); err != nil {
		t.Fatalf("unmarshal list: %v", err)
	}
	t.Logf("TestE2E_PaymentTerms_List: %d items returned", len(items))
}

// TestE2E_PaymentTerms_Get discovers a payment_term id via List and fetches
// its detail, applying strict field diff on the single-object response.
func TestE2E_PaymentTerms_Get(t *testing.T) {
	client := newE2EClient(t)
	ctx := context.Background()
	...
	if len(items) == 0 {
		...
		t.Skipf("payment_terms list returned 0 items; Get pending re-verification (see roadmap Pending Re-verification)")
	}
	...
	dumpJSON(t, "payment_terms", id, getRaw)

	if diff := testhelper.StrictFieldDiff(t, getRaw, &boardapi.PaymentTermEntity{}); len(diff) > 0 {
		t.Errorf("payment_terms get(%d) unmapped fields: %v", id, diff)
	}

	var got boardapi.PaymentTermEntity
	if err := json.Unmarshal(getRaw, &got); err != nil {
		t.Fatalf("unmarshal get: %v", err)
	}
	if got.ID != id {
		t.Errorf("GetPaymentTerm ID mismatch: got=%d want=%d", got.ID, id)
	}
	t.Logf("TestE2E_PaymentTerms_Get: id=%d name=%q", got.ID, got.Name)
}

// TestE2E_PaymentTerms_Search exercises Search with a non-matching name and
...
func TestE2E_PaymentTerms_Search(t *testing.T) {
	client := newE2EClient(t)
	ctx := context.Background()
	...
	if diff := testhelper.StrictFieldDiff(t, raw, &[]boardapi.PaymentTermEntity{}); len(diff) > 0 {
		t.Errorf("payment_terms search unmapped fields: %v", diff)
	}
	...
	t.Logf("TestE2E_PaymentTerms_Search: %d items returned", len(items))
}
